...
There is **no separate bin** — the package ships only the `mcp-jira-confluence` bin (`dist/index.js`). `index.ts` routes to the CLI before its server bootstrap runs: a top-level guard calls `isCliInvocation(process.argv.slice(2))` (exported from `cli.ts` — true when the first arg is `confluence`/`jira` or `--help`/`-h`) and, if so, `process.exit(await run(argv))` without ever starting a transport or loading the server's two-config bootstrap. So the CLI is reached as `mcp-jira-confluence <group> <command>` (e.g. `npx -y mcp-jira-confluence@latest jira get-issue PROJ-1`). `cli.ts` is a thin argv→function adapter over the same `confluence.ts` / `jira.ts` layer (the `npm run cli` / `dev:cli` scripts still run `dist/cli.js` directly for local dev, guarded by `isInvokedAsScript`):
